...
from layerbase import Model, SymbolDataLayer, safefile, DrawPatch, LogSoftmaxLayer, LabelLoss, makesoftmaxlabel
from hengelossvalid import binaryloss
...
if __name__=="__main__":
    ...
    data=np.load('alignedallface.npz')
    inp=data['input']
    oup=data['output']
    data = None
    ...
    obinary,_tp,_fp,_tn,_fn,_F = binaryloss(l1_7sm.output, lout.output, -1e-5, 0)
    ...
    for param, momentum in zip(params, momentums):
        updates.append((param, param*0.99999 + momentum))
    ...
    if len(sys.argv)>1 and sys.argv[1] == 'test':
        ...
        def processarray(arraystr):
            ...
            try:
                ...
                b,c,d=np.copy(arr)[0]
                arr[0,:]=[d,c,b]
                np.save('pallface.npy',arr)
                arr = (arr-arr.mean(axis=(2,3),keepdims=True))/(arr.std(axis=(2,3),keepdims=True)+1e-10)
                output = vis_model(arr,oups)[-2]
                np.save('pallfaceout.npy',output)
                print output.shape
    ...
    for i in range(1000000):
        ...
        if i % BATCHSTEP == BATCHSTEP-1:
            ...
            for i in model.paramlayers():
                ...
                if len(param.shape)!=4:
                    if hasattr(i,'reshape') and i.reshape!=None:
                        PIL.Image.fromarray(DrawPatch(param.reshape((-1,)+i.reshape[1:]))).save('stepallface/layer_%s.jpg'%layer, quality=100)
                    else:
                        PIL.Image.fromarray(np.array((param-np.min(param))/(np.max(param)-np.min(param))*255,np.uint8)).save('stepallface/layer_%s.png'%layer)
                    continue
                PIL.Image.fromarray(DrawPatch(param)).save('stepallface/layer_%s.jpg'%layer, quality=100)
            #Draw response
            ...
            for i in resp:
                layer += 1
                if len(i.shape)!=4:
                    PIL.Image.fromarray(np.array((i-np.min(i))/(np.max(i)-np.min(i))*255,np.uint8)).save('stepallface/resp%s.png'%layer)
                    continue
                PIL.Image.fromarray(DrawPatch(i[0:1])).save('stepallface/resp%s.jpg'%layer, quality=100)
        ...
        b = npr.randint(l0.n_batches)
        #Save model
        with safefile('modelfallface2') as savef:
            model.save(savef.wb())
