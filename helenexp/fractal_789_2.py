    ...
    if len(sys.argv)>1 and sys.argv[1] == 'test':
        ...
        def getF(ab):
            ...
            for j in range(100/BS):
                ...
                for i in resp:
                    layer += 1
                    if len(i.shape)!=4:
                        PIL.Image.fromarray(np.array((i-np.min(i))/(np.max(i)-np.min(i))*255,np.uint8)).save('stepf789/%s_resp%s.png'%(j,layer))
                        continue
                    PIL.Image.fromarray(DrawPatch(i[0:1])).save('stepf789/%s_resp%s.jpg'%(j,layer), quality=100)
                PIL.Image.fromarray(DrawMaskedPatch(resp[0][0:1], resp[-1][0:1,0:1])).save('stepf789/maskresult_%s.jpg'%(j), quality=100)
                PIL.Image.fromarray(DrawMaskedPatch(resp[0][0:1], resp[-2][0:1,0:1])).save('stepf789/masktruth_%s.jpg'%(j), quality=100)
                #v=v[:-1]
                print v
                pr.append(v)
            ...
            print ab,F
            return F[:-1].sum()
        print getF(np.array((  7.37406694 , 1.51592342),'f'))
        #from scipy.optimize import fmin
        #print fmin(getF,np.array(( 7.4096077,1.48307174),'f')).xopt
        sys.exit(0)
        ...
        def processarray(arraystr):
            ...
            try:
                ...
                arr = np.array(np.fromstring(rawarray, dtype=np.uint8).reshape((1,3,80,80)),'f')
                b,c,d=np.copy(arr)[0]
                arr[0,:]=[d,c,b]
                np.save('p789.npy',arr)
                arr = (arr-arr.mean(axis=(2,3),keepdims=True))/(arr.std(axis=(2,3),keepdims=True)+1e-10)
                output = vis_model(arr,oups)[-3]
                np.save('p789out.npy',output)
                print output.shape
    ...
    for i in range(1000000):
        ...
        if i % BATCHSTEP == BATCHSTEP-1:
            ...
            for i in model.paramlayers():
                ...
                if len(param.shape)!=4:
                    if hasattr(i,'reshape') and i.reshape!=None:
                        PIL.Image.fromarray(DrawPatch(param.reshape((-1,)+i.reshape[1:]))).save('stepf789/layer_%s.jpg'%layer, quality=100)
                    else:
                        PIL.Image.fromarray(np.array((param-np.min(param))/(np.max(param)-np.min(param))*255,np.uint8)).save('stepf789/layer_%s.png'%layer)
                    continue
                PIL.Image.fromarray(DrawPatch(param)).save('stepf789/layer_%s.jpg'%layer, quality=100)
            #Draw response
            ...
            for i in resp:
                layer += 1
                if len(i.shape)!=4:
                    PIL.Image.fromarray(np.array((i-np.min(i))/(np.max(i)-np.min(i))*255,np.uint8)).save('stepf789/resp%s.png'%layer)
                    continue
                PIL.Image.fromarray(DrawPatch(i[0:1])).save('stepf789/resp%s.jpg'%layer, quality=100)
        ...
        b = npr.randint(l0.n_batches)
        #Save model
        with safefile('modelf789_2') as savef:
            model.save(savef.wb())
