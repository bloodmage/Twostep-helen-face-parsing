...
if __name__=="__main__":
    ...
    with safefile('modelf23_2') as loadf:
        if loadf:
            model.load(loadf.rb())
        ...
        def getF(ab):
            ...
            for j in range(200/BS):
                ...
                for i in resp:
                    layer += 1
                    if len(i.shape)!=4:
                        #PIL.Image.fromarray(np.array((i-np.min(i))/(np.max(i)-np.min(i))*255,np.uint8)).save('stepf23/%s_resp%s.png'%(j,layer))
                        continue
                    #PIL.Image.fromarray(DrawPatch(i[0:1])).save('stepf23/%s_resp%s.jpg'%(j,layer), quality=100)
                #PIL.Image.fromarray(DrawMaskedPatch(resp[0][0:1], resp[-1][0:1,0:1])).save('stepf23/maskresult_%s.jpg'%(j), quality=100)
                #PIL.Image.fromarray(DrawMaskedPatch(resp[0][0:1], resp[-2][0:1,0:1])).save('stepf23/masktruth_%s.jpg'%(j), quality=100)
                #v=v[:-1]
                print v
                pr.append(v)
            ...
            print ab,F
            return F[0]
        #print getF(np.array((3.73503632,1.48109698),'f'))
        from scipy.optimize import fmin
        print fmin(getF,np.array((3.73503632,1.48109698),'f')).xopt
        sys.exit(0)
    ...
    for i in range(1000000):
        ...
        if i % BATCHSTEP == BATCHSTEP-1:
            ...
            for i in model.paramlayers():
                ...
                if len(param.shape)!=4:
                    if hasattr(i,'reshape') and i.reshape!=None:
                        PIL.Image.fromarray(DrawPatch(param.reshape((-1,)+i.reshape[1:]))).save('stepf23/layer_%s.jpg'%layer, quality=100)
                    else:
                        PIL.Image.fromarray(np.array((param-np.min(param))/(np.max(param)-np.min(param))*255,np.uint8)).save('stepf23/layer_%s.png'%layer)
                    continue
                PIL.Image.fromarray(DrawPatch(param)).save('stepf23/layer_%s.jpg'%layer, quality=100)
            #Draw response
            ...
            for i in resp:
                layer += 1
                if len(i.shape)!=4:
                    PIL.Image.fromarray(np.array((i-np.min(i))/(np.max(i)-np.min(i))*255,np.uint8)).save('stepf23/resp%s.png'%layer)
                    continue
                PIL.Image.fromarray(DrawPatch(i[0:1])).save('stepf23/resp%s.jpg'%layer, quality=100)
        ...
        b = npr.randint(l0.n_batches)
        #Save model
        with safefile('modelf23_2') as savef:
            model.save(savef.wb())
