...
inputs = np.zeros((len(files),3,64,64),np.uint8)
outputs = np.zeros((len(files),8,64,64),'f')
...
for i in files:
    ...
    for oid in range(2,10):
        outputs[idx,oid-2] = fromimage(Image.open(StringIO(zf.read('allface/%s_0%s.png'%(name,oid)))))
...
np.savez('allface2.npz',input=inputs,output=outputs)
